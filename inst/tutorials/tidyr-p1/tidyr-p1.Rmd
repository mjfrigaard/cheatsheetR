---
title: "Data tidying with tidyr (part 1)"
output: 
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
The third tenet is a logical extension from first two 1 & 2, but it's not always explicitly stated. I've included it here because it always reminds me what the data in a tidy format contains. I like this quote from the [tidy data paper](https://doi.org/10.18637/jss.v059.i10) by Hadley Wickham, 
...
```{r tibble-vectors, exercise=TRUE, exercise.lines=5}
...
```{r tribble-complex-names, exercise=TRUE, exercise.lines=5}
...
```{r enframe-five_babies, exercise=TRUE, exercise.lines=2, exercise.setup="prepare-five_babies"}
...
In the code block below we've loaded the `smiths` data from the `tidyr` package. Print `smiths` to the console: 

```{r prepare-smiths, echo=FALSE}
library(tidyr)
smiths <- tibble::tribble(
...
```{r smiths, exercise=TRUE, exercise.lines=5, exercise.setup="prepare-smiths"}
# print 'smiths' to the console

```

We've also loaded these data as a `data.frame` as `smithsdf`

```{r smithsdf, exercise=TRUE, exercise.lines=5, exercise.setup="prepare-smiths"}
# print 'smithsdf' to the console

```

In the code block below, convert `smithsdf` to a `tibble` using `tibble::as_tibble()`. 

```{r as-tibble, exercise=TRUE, exercise.lines=5, exercise.setup="prepare-smiths"}
# drop 'smithsdf' in the as_tibble() function
tibble::as_tibble()
```

Note the two differences in the `tibble` outputs above: 

1. `tibble`'s print the structure (`A tibble:`) 
2. `tibble`s print the dimensions of the object (`2 × 5`)   
2. We can also see the class or type of each variable (`<chr>`, `<dbl>`, etc.) in the `tibble`.    
...
What does "*Only values of size one are recycled*" mean? It means if `x` is a single value, it will be repeated for the length of `y`---which is why the code below works:
...
Datasets with a single variable in multiple columns are referred to as having a 'wide' format. See the example (`table4a`) below: 
...
```{r pivot_longer-1-table4a, exercise=TRUE, exercise.lines=9, exercise.setup="prepare-table4a"}
tidyr::pivot_longer(
...
```{r test-pivot_longer, echo=FALSE, eval=FALSE}
tidyr::pivot_longer(
  data = table4a, 
  cols = 2:3, 
  names_to = "year", 
...
```{r names_from, exercise=TRUE, exercise.lines=5, exercise.setup="prepare-table2"}
...
```{r test-pivot_wider, echo=FALSE, eval=FALSE}
pivot_wider(
  data = table2, 
  names_from = type, 
  values_from = count)
...
bbwk2 <- tibble::tribble(
  ~date.entered,        ~artist,              ~track, ~wk1, ~wk2,
...
# use head() or tail() to view the bbwk2 data
```
...
```{r pivot_longer-bbwk3, exercise=TRUE, exercise.lines=8, exercise.setup="prepare-bbwk3"}
...
```{r unite, exercise=TRUE, exercise.lines=9, exercise.setup="prepare-unite"}
...
```{r test-unite, echo=FALSE, eval=FALSE}
unite(
  data = table5, 
   century, year, 
   col = "year", 
...
```{r separate, exercise=TRUE, exercise.lines=8, exercise.setup="prepare-table3"}
...
```{r test-separate, echo=FALSE, eval=FALSE}
separate(
  data = table3, 
  col = rate, 
  into = c('cases', 'pop'), 
...
3. The `regex` argument is a regular expression pattern that must be capable of creating "groups"  
4. The groups produced by the regex pattern will make up the values in the new columns we supply to the `into` argument  
...
```{r extract, exercise=TRUE, exercise.lines=7, exercise.setup="prepare-table3"}
extract(table3, 
...
```{r test-extract, echo=FALSE, eval=FALSE}
extract(table3, 
        col = rate, 
        into = c('cases','pop'), 
        regex = "([[:alnum:]]+)/([[:alnum:]]+)")
...
Use the code block below to separate the `rate` column on `/` down by rows instead of across columns in `table3`.

```{r separate_rows, exercise=TRUE, exercise.lines=5, exercise.setup="prepare-table3"}
separate_rows(data = table3, 
              , # column to separate values below
              sep = "")
```

```{r test-separate_rows, echo=FALSE, eval=FALSE}
separate_rows(
  data = table3, 
  rate, 
  sep = "/")
...
We've recreated the example dataset from the cheatsheet (`X`) below:

```{r prepare-missing-X, echo=FALSE}
X <- tibble::tribble(
  ~x1,   ~x2,
  "A",    1L,   
  "B",    NA,   
...
```{r explore-X, exercise=TRUE, exercise.lines=3, exercise.setup="prepare-missing-X"}
...
```{r drop_na, exercise=TRUE, exercise.lines=3, exercise.setup="prepare-missing-X"}
drop_na(data = )
```

```{r test-drop_na, echo=FALSE, eval=FALSE}
drop_na(data = X)
```
...
If we don't want to remove the entire row, we can use `tidyr::fill()` to replace the missing values with the column value's from above or below.

In the code block below: 

1. Place the `X` dataset in the data argument  
2. Fill the missing values in the `x2` column with the values in the cells above 

```{r fill, exercise=TRUE, exercise.lines=3, exercise.setup="prepare-missing-X"}
fill(data = , )
```

```{r test-fill_na, echo=FALSE, eval=FALSE}
fill(data = X, x2)
```


We can see the missing values in `x2` have been filled with the contents from the values above. The `.direction` argument controls which values are used to replace the missing values. `"down"` (the default) will fill the missing values with the contents above, and `"up"` will fill the missing values with the contents from below. We've loaded another example dataset, profits, in the code block below.
...
```{r fill-up, exercise=TRUE, exercise.lines=5, exercise.setup="prepare-profits"}
# fill up!
fill(data = , 
     , 
     .direction = 
...
```{r test-fill-up, echo=FALSE, eval=FALSE}
fill(data = X, x2, .direction = 'up')
```
...
Below is the example from the cheatsheet using the `X` dataset.

```{r replace_na, exercise=TRUE, exercise.lines=3, exercise.setup="prepare-missing-X"}
...
```{r replace_na-scores, exercise=TRUE, exercise.lines=3, exercise.setup="prepare-cumdf"}
replace_na(data = , 
           replace = list( = ))
```
...
For now, assume this column has been created like the `cum_scores` data below:

```{r explore-cum_scores, exercise=TRUE, exercise.lines=3, exercise.setup="prepare-cumdf"}
# explore cum_scores

```

Use the code block below to recreate the following `tibble` using the appropriate functions (`drop_na()`, `fill()`, or `replace_na()`): 
...
```{r fix-cum_scores, exercise=TRUE, exercise.lines=3, exercise.setup="prepare-cumdf"}
# replace missing total values

```


```{r answer-replace_na-scores, eval=FALSE, echo=FALSE}
...
The `tidyr` cheatsheet also uses another `X` dataset for this section (which we've created again below). Use the code block below to explore X:

```{r prepare-expand-X, echo=FALSE}
X <- tibble::tribble(
  ~x1,   ~x2,    ~x3,
  "A",    1L,     3L,
  "B",    1L,     4L,
...
```{r explore-expand-X, exercise=TRUE, exercise.lines=3, exercise.setup="prepare-expand-X"}
# explore the X data
```
...
1. Place `X` in the `data` argument  
2. Expand `X` to include all possible combinations of variables `x1` and `x2`. 

```{r expand, exercise=TRUE, exercise.lines=3, exercise.setup="prepare-expand-X"}
expand(data = )
```

```{r test-expand, include=FALSE, eval=FALSE}
expand(data = X, x1, x2)
```

The `expand()` function returns a `tibble` with all the possible combinations of values for whichever variables we provide *after* the `data` argument (in this case `x1` and `x2`). You can check the new combinations of values that have been added using [`anti_join()`](https://dplyr.tidyverse.org/reference/filter-joins.html) and [`distinct()`](https://dplyr.tidyverse.org/reference/distinct.html) from the `dplyr` package. 

```{r anti_join-distinct, exercise=TRUE, exercise.lines=5, exercise.setup="prepare-expand-X"}
dplyr::anti_join(
...
Let's see how to use complete by looking at the example from the cheatsheet, `X`. 

```{r prepare-complete-X, echo=FALSE}
X <- tibble::tribble(
  ~x1,   ~x2,    ~x3,
  "A",    1L,     3L,
  "B",    1L,     4L,
...
```{r complete-display-X, echo=FALSE}
X <- tibble::tribble(
  ~x1,   ~x2,    ~x3,
  "A",    1L,     3L,
  "B",    1L,     4L,
...
We can see there are three variables, each with two possible values. If we provide the same two columns to `complete()` that we did to expand, we can see the difference below:

```{r complete, exercise=TRUE, exercise.lines=5, exercise.setup="prepare-complete-X"}
complete(data = , )
```
...
```{r prepare-hiphop90s, echo=FALSE}
hiphop90s <- tribble(
  ~year, ~artist, ~albums,
  1996, "2Pac", 2L,
...
This table shows hip-hop albums for three artists from year 1996-1998. If we wanted to fill in the missing years for each artist, how would we do it with `complete()`?


```{r complete-hiphop90s, exercise=TRUE, exercise.lines=5, exercise.setup="prepare-hiphop90s"}
complete(data = hiphop90s, )
```

We can also use the `fill` argument in `complete()` to replace any missing values we generate. Use the code block below to replace the missing values in `albums` with zeros:

```{r complete-fill, exercise=TRUE, exercise.lines=5, exercise.setup="prepare-hiphop90s"}
complete(data = hiphop90s, 
  , , 
  fill = list( = 0L))
```
...
```{r complete-non-present, exercise=TRUE, exercise.lines=7, exercise.setup="prepare-hiphop90s"}
complete(hiphop90s,
  year,
...
```{r complete-non-present-answer, echo=FALSE, eval=FALSE}
complete(hiphop90s,
  year,
  # add Dre to the list of artists
